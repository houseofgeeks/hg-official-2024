...
const RequestsPage = () => {
  ...
  useEffect(() => {
    const getRequests = async () => {
      try {
        setLoading(true);
        const res = await axios.get("/api/v1/requests/SD/pending", {
        ...
        <Card className="w-full mt-4 sm:mt-8 md:mt-20 border border-gray-200 shadow-lg bg-white">
          <CardHeader className="p-6 rounded-t-lg border-b border-[#9A43B1]/30">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="secondary" className="text-sm px-3 py-1 bg-[#9A43B1]/20 text-[#9A43B1]">
              ...
              {requests.length === 0 ? (
                <Card className="bg-gray-50 p-12 text-center">
                  <div className="flex flex-col items-center space-y-3">
                    ...
                    <p className="text-gray-600">There are no pending level upgrade requests at this time.</p>
                  </div>
                </Card>
              ) : (
                requests.map((request) => (
                  <Card key={request.id} className="bg-gray-50 hover:bg-gray-100 transition-colors duration-200">
                      ...
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-6">
                        <div className="space-y-4 flex-1">
                          <div className="flex flex-wrap items-start gap-3">
                            ...
                            <Badge 
                              className={`px-3 py-1 border ${getStatusColors(request.status)}`}
                            >
